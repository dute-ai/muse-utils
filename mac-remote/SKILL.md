...
6. The helpers resolve config from env vars first, then `<state-dir>/config`. The state dir is `state/` inside the utility checkout (created by `install.sh`, gitignored); installed copies read the install-time path from `<skill>/.state-dir`. Older installs fall back to `~/workspace/muse-utils/state`, then `~/workspace/mac-remote/state`. Override everything with `MAC_REMOTE_STATE_DIR`.
